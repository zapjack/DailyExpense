...
public class MyChart {
    ...
    public static FlowPane makeChart(String [] seriesNames, int [] firstSeries, int [] secondSeries) {
        ...
        for (int i = 0; i < seriesNames.length; i++)
            income.getData().add(Helper.dat(seriesNames[i], secondSeries[i]));
        
        income.getData().add(Helper.dat(TOTAL_LABEL, 0));
        
        expenses.setName("Expenses");
        for (int i = 0; i < seriesNames.length; i++)
            expenses.getData().add(Helper.dat(seriesNames[i], firstSeries[i]));
        ...
        int total = 0;
        for (int i = 0; i < seriesNames.length; i++) {
            int sum = secondSeries[i] + firstSeries[i];
            total += sum;
            net.getData().add(Helper.dat(seriesNames[i], sum));
        }
